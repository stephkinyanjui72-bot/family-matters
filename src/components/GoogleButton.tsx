...
// "Continue with Google" button. Only works after you enable Google as an
// OAuth provider in the Supabase dashboard (Authentication → Providers →
// Google) and paste your Google Cloud Console client ID + secret.
export function GoogleButton({ redirectTo = "/" }: { redirectTo?: string }) {
  ...
  const onClick = async () => {
    ...
    const { error } = await sb.auth.signInWithOAuth({
      provider: "google",
      options: {
        // Supabase bounces us through its hosted callback first, then to this.
    ...
    // Successful flow: the browser navigates to Google. No need to clear busy.
  };
